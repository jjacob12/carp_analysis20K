---
title: "ONS76-Spher_Seurat_analysis_QC1_20K"
author: "JJ"
date: "07/07/2022"
...
# note these are .gz files which can be directly read. There are 1334 cells.

ons76spher.data <- ReadMtx(mtx = paste0(data_dir, "matrix.mtx.gz"),
...
# Minimum genes per cell is 129, and median expression is 3492.

at_least_one <- apply(ons76spher.data, 2, function(x) sum(x>0)) # counts the sum of rows that have a                                                                        count value (x > 0) for each column (counts                                                                  the number of TRUE values per column)
summary(at_least_one)

# summary of total expression per cell.
summary(colSums(ons76spher.data))
```
...
hist(colSums(ons76spher.data),                             # total counts (expression) per cell
...
# ~17.7K genes are expressed in at least 3 cells
# ~18.8K genes are not expressed in at least 3 cells
tmp <- apply(ons76spher.data, 1, function(x) sum(x > 0))
table(tmp >= 3)
```

```{r cells_gene_counts}
# Do all 1334 cells have at least 200 expressed genes? Ans = NO! Min. genes = 129

keep <- tmp>=3                  # for each gene this states TRUE or FALSE

tmp2 <- ons76spher.data[keep, ]        # subset ons76spher.data to keep only TRUE values, i.e. genes which 
                                       # are expressed in 3 or more cells

class(tmp2)                     # this is a matrix
dim(tmp2)                       # dim is 17719 by 1334, as, for example for some genes the 1334th cell may
...
genes_per_cell <- Matrix::colSums(ons76spher.data > 0) # count gene only if it has non-zero reads                                                                   mapped.
...
ons76spher.seurat <- CreateSeuratObject(counts = ons76spher.data, 
                                    min.cells = 3, 
                                    min.genes = 200, 
                                    project = "ons76spher")
...
# number of cells with percent.mt > 10%: 205 cells
# number of cells with percent.mt < 10%: 1129 cells
...
```{r extract_countsSlot_1129cells}
# subset '@counts' slot (matrix) of filtered seurat object

counts.1129 <- GetAssayData(object = filt.ons76spher.seurat, slot = "counts")
```

```{r summarise_stats_1129cells}
# median genes per cell and expression sum, as above. Min. genes = 357

at_least_one.1129 <- apply(counts.1129, 2, function(x) sum(x>0))

summary(at_least_one.1129)

summary(colSums(counts.1129))
```
...
hist(at_least_one.1129, breaks = 100,
     main = "Distribution of detected genes",
     xlab = "Genes with at least one transcript")

hist(colSums(counts.1129),                             # total counts (expression) per cell
     breaks = 100, main = "Expression sum per cell",
     xlab = "Sum expression")
```

```{r Plots_UMI_gene_expression_1129cells, fig.height=10, fig.width=5}

# looks ok, most of the filtered ons76spher cells have UMI counts > 10,000 and express 3000-4000 genes
...
# 14.6K genes compared to 17.7K genes (latter is from code line 195)
dim(filtCellsGenes.ons76spher.seurat)
```

```
# saved the 'filtCellsGenes.ons76spher.seurat' object to outputs20K folder

saveRDS(filtCellsGenes.ons76spher.seurat, file = "./outputs20K/filtCellsGenes.ons76spher.seurat.rds")
```
...
fullFilt.ons76spher.seurat <- readRDS("outputs20K/filtCellsGenes.ons76spher.seurat.rds")
```
...
```{r Plot_PCA}
# ONS76 spheroid cells do not obviously segregate by cell-cycle phase.
...
```
saveRDS(seurat.phase, "./outputs20K/seurat.phase.ons76spher.rds")
```
