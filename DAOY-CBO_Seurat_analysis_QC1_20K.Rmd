---
title: "DAOY-CBO_Seurat_analysis_QC1_20K.Rmd"
author: "JJ"
date: "08/07/2022"
...
# note these are .gz files which can be directly read. There are 3479 cells and 36,601 genes.
...
# Minimum genes per cell is 79.
# Median expression per cell is 1632 genes which is on a par with ONS76-CBO sample
...
# ~16.2K genes are not expressed in at least 3 cells
...
```{r cells_gene_counts}
# Do all 3479 cells have at least 200 expressed genes? Ans = NO! Min. genes = 79

keep <- tmp>=3                       # for each gene this states TRUE or FALSE

tmp2 <- daoyCbo.data[keep, ]        # subset ons76cbo.data to keep only TRUE values, i.e. genes which 
...
# looks similar to ONS76-CBO sample plot
...
# Seurat object has ~20K genes and 3479 cells
...
```{r metadata_dataframe_3321cells}
...
```{r violinPlot_genes_transcripts_percent.mt_3479cells, fig.height=10, fig.width=5}
...
```{r Plot_UMIcountsPerCell_3479cells}

# visualise number of UMIs (transcripts) per cell using all 3479 cells. 
...
# number of cells with percent.mt > 10%: 720 cells
# number of cells with percent.mt < 10%: 2759 cells
...
# CELL LEVEL FILTERING - leaves ~ 20K genes and 2759 cells
#-------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------
...
```{r extract_countsSlot_2759cells}
# subset '@counts' slot (matrix) of filtered seurat object

counts.2759 <- GetAssayData(object = filt.daoyCbo.seurat, slot = "counts")
```

```{r summarise_stats_2759cells}
# median genes per cell and expression sum, as above. Min. genes = 203. Median = 1827.
...
at_least_one.2759 <- apply(counts.2759, 2, function(x) sum(x>0))

summary(at_least_one.2759)

summary(colSums(counts.2759))
```

```{r Plots_GenesPerCell_2759cells, fig.height=10, fig.width=8}
...
hist(at_least_one.2759, breaks = 100,
     main = "Distribution of detected genes",
     xlab = "Genes with at least one transcript")

hist(colSums(counts.2759),                             # total counts (expression) per cell
     breaks = 100, main = "Expression sum per cell",
     xlab = "Sum expression")
```

```{r Plots_UMI_gene_expression_2759cells, fig.height=10, fig.width=5}

# looks ok, most of the filtered cells have UMI counts >3000 and <10,000 and express 1000-3000 genes, which is similar to ONS76-CBO sample
...
dim(filtCellsGenes.daoyCbo.seurat)           # number of genes further reduced to ~ 16K genes vs 20K genes                                                 for only cell-level filtering
```

```
# saved the 'filtCellsGenes.daoyCbo.seurat' object to outputs20K folder

saveRDS(filtCellsGenes.daoyCbo.seurat, file = "./outputs20K/filtCellsGenes.daoyCbo.seurat.rds")
```
...
fullFilt.daoyCbo.seurat <- readRDS("outputs20K/filtCellsGenes.daoyCbo.seurat.rds")
```

```{r SCTransform}
# apply the SCTransform algorithm

fullFilt.daoyCbo.seurat <- SCTransform(fullFilt.daoyCbo.seurat, method = "glmGamPoi", verbose = FALSE)
```

```{r RunPCA}
fullFilt.daoyCbo.seurat <- RunPCA(fullFilt.daoyCbo.seurat, verbose = FALSE)
```

```{r ElbowPlot_ndims=50}
# elbow plot shows that there could be an elbow at PC 15, but the graph does not level off after
# PC 15 showing there is some additional information to ~ PC 40. Maybe not too surprising given
# the complexity and diversity of cell types in this sample.
ElbowPlot(fullFilt.daoyCbo.seurat, ndims = 50, reduction = "pca")
```
...
# Plot the PCA colored by cell cycle phase - cell cycle has no obvious effect on clustering
...
```{r Plot_PCA}
# change nFeatures to 4000 - still no strong clustering by cell-cycle phase, but check if
# cell-cycle should be regressed out
seurat.phase <- FindVariableFeatures(seurat.phase, 
                     selection.method = "vst",
                     nfeatures = 2000, 
                     verbose = FALSE)
...
# Plot the PCA colored by cell cycle phase - cell cycle has no strong effect on clustering
DimPlot(seurat.phase,
        reduction = "pca",
        group.by= "Phase")
```
...
```{r Plot_PCA}
# This time the 'split.by' argument is added - cells do not obviously segregate by cell-cycle phase.
...
saveRDS(seurat.phase, "./outputs20K/seurat.phase.daoyCbo.rds")
```
