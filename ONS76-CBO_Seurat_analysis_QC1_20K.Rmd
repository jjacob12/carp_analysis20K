---
title: "ONS76-CBO_Seurat_analysis_QC1_20K"
author: "JJ"
date: "08/07/2022"
...
# note these are .gz files which can be directly read. There are 3321 cells and 36601 genes.
...
# Minimum genes per cell is 108.
# Median expression per cell is 1627 genes is on a par with CBO alone and ONS76-Spher alone
...
# ~16.5K genes are not expressed in at least 3 cells
...
```{r cells_gene_counts}
# Do all 3321 cells have at least 200 expressed genes? Ans = NO! Min. genes = 108
...
# Seurat object has ~20K genes and 3321 cells
...
```{r violinPlot_genes_transcripts_percent.mt_3321cells, fig.height=10, fig.width=5}
...
```{r Plot_UMIcountsPerCell_3321cells}

# visualise number of UMIs (transcripts) per cell using all 3321 cells. 
...
# number of cells with percent.mt > 10%: 1069 cells
# number of cells with percent.mt < 10%: 2252 cells
...
# CELL LEVEL FILTERING - leaves 20K genes and 2252 cells
#-------------------------------------------------------------------------------------------
#-------------------------------------------------------------------------------------------
...
```{r extract_countsSlot_2252cells}
# subset '@counts' slot (matrix) of filtered seurat object

counts.2252 <- GetAssayData(object = filt.ons76cbo.seurat, slot = "counts")
```

```{r summarise_stats_2252cells}
# median genes per cell and expression sum, as above. Min. genes = 253. Median = 2028.
...
at_least_one.2252 <- apply(counts.2252, 2, function(x) sum(x>0))

summary(at_least_one.2252)

summary(colSums(counts.2252))
```

```{r Plots_GenesPerCell_2252cells, fig.height=10, fig.width=8}
...
hist(at_least_one.2252, breaks = 100,
     main = "Distribution of detected genes",
     xlab = "Genes with at least one transcript")

hist(colSums(counts.2252),                             # total counts (expression) per cell
     breaks = 100, main = "Expression sum per cell",
     xlab = "Sum expression")
```

```{r Plots_UMI_gene_expression_2252cells, fig.height=10, fig.width=5}

# looks ok, most of the filtered cells have UMI counts >3000 and <10,000 and express 1000-3000 genes
...
dim(filtCellsGenes.ons76cbo.seurat)           # number of genes further reduced to ~ 16K genes vs 20K genes                                                 for only cell-level filtering
```

```
# saved the 'filtCellsGenes.ons76cbo.seurat' object to outputs20K folder

saveRDS(filtCellsGenes.ons76cbo.seurat, file = "./outputs20K/filtCellsGenes.ons76cbo.seurat.rds")
```
...
fullFilt.ons76cbo.seurat <- readRDS("outputs20K/filtCellsGenes.ons76cbo.seurat.rds")
```
...
```
saveRDS(seurat.phase, "./outputs20K/seurat.phase.ons76cbo.rds")
```
