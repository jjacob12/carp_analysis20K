...
```{r metadata_dataframe_3729cells}
...
```{r Plot_UMIcountsPerCell_3729cells}
...
```
# saved the filtered seurat object to outputs20K folder - this file was overwritten!
...
```
# saved the filteredCellsGenes.cbo.seurat object to outputs20K folder - **filename overwrites
# the previously saved cell-filtered seurat object of the same name

saveRDS(filteredCellsGenes.cbo.seurat, file = "./outputs20K/filtered.cbo.seurat.rds")
```
...
# number of genes (16,624) is less after gene-level filtering 
# (vs 20,479 genes upon cell-level filtering)
...
# PC 15 showing there is some additional information to ~ PC 30.

ElbowPlot(filtered.cbo.seurat, ndims = 50, reduction = "pca")
```

```{r extract_countsSlot_2757cells}
# subset '@counts' slot (matrix) of filtered seurat object
counts.2757.v2 <- GetAssayData(object = filteredCellsGenes.cbo.seurat, slot = "counts")
counts.2757.v2[1:6, 1:3]
```
